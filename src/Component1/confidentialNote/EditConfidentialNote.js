...
export default function EditConfidentialNote() {
    ...
    blocks: [
      {
        ...
        type: "unstyled", // or other block types like 'header-one', 'unordered-list-item', etc.
        depth: 0,
        inlineStyleRanges: [],
        entityRanges: [],
        data: {},
  ...
  const onEditorStateChange = newEditorState => {
    ...
    setFormValues(prev => ({
      ...prev,
      description: newEditorState.getCurrentContent().getPlainText(),
    }));
  ...
  const handleSubmit = () => {
    axiosConfig
      .put(
        `/confidential/update-confidential/${location?.state._id}`,
        formValues
      )
      .then(response => {
